...
class GninaResultRepository:
    @property
    def collection(self):
        return get_database()["gnina_results"]
    ...
    async def list_results(
        ...
        cursor = self.collection.find(query).sort("rank", pymongo.ASCENDING).skip(skip).limit(limit)
        items = await cursor.to_list(length=limit)
        return items, total

gnina_result_repository = GninaResultRepository()
